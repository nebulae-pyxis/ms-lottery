...
import { LotteryListService } from './lottery-list.service';
...
export class LotteryListComponent implements OnInit, OnDestroy {
  ...
  updateFilterDataSubscription() {
    this.listenFilterFormChanges$()
      ...
      .subscribe(filterData => {
        ...
        this.LotteryListservice.updateFilterData(filterData);
      });
  }
  ...
  updatePaginatorDataSubscription() {
    this.listenPaginatorChanges$()
      ...
      .subscribe(pagination => {
        ...
        this.LotteryListservice.updatePaginatorData(paginator);
      });
  }
  ...
  loadLastFilters() {
    combineLatest(
      this.LotteryListservice.filter$,
      this.LotteryListservice.paginator$
    ).pipe(
      take(1)
  ...
  refreshTableSubscription() {
    combineLatest(
      ...
      mergeMap(([filterInput, paginationInput]) => {
        return forkJoin(
          this.getlotteryList$(filterInput, paginationInput),
          this.getlotterySize$(filterInput),
        )
      }),
      takeUntil(this.ngUnsubscribe)
    )
   ...
   */
  getlotteryList$(filterInput, paginationInput){
    return this.LotteryListservice.getlotteryList$(filterInput, paginationInput)
    .pipe(
      mergeMap(resp => this.graphQlAlarmsErrorHandler$(resp)),
      map(resp => resp.data.LotteryLotteries)
    );
  }

    /**
   * Gets the lottery size
   * @param filterInput 
   */
  getlotterySize$(filterInput){
    return this.LotteryListservice.getlotterySize$(filterInput)
    .pipe(
      mergeMap(resp => this.graphQlAlarmsErrorHandler$(resp)),
      map(resp => resp.data && resp.data.LotteryLotteriesSize ? resp.data.LotteryLotteriesSize: 0)
    );
  }

  /**
   * Receives the selected lottery
   * @param lottery selected lottery
   */
  selectlotteryRow(lottery) {
  ...
  goToDetail(){
    ...
    .pipe(
      ...
      if(selectedBusiness == null || selectedBusiness.id == null){
        ...
        this.router.navigate(['lottery/new']);
      }      
    })    
  }
